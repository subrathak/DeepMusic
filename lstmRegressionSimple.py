...
with open('./Data/segmentEngagementGuru.pickle', 'rb') as handle:
...
def getSlices(x,y):
    ...
    for i in range(0,cutoff,maxlen):
        seg = x[:,i:i+maxlen]
        sliceX.append(seg.T)
...
model = Sequential()
model.add(LSTM(30, batch_input_shape=(batch_size,maxlen,input_dim)))
model.add(Dense(1))
...
# plt.figure()
# plt.subplot(211)
# plt.plot(movingaverage(preds[:,0],10))
# plt.title("Preds")
# plt.subplot(212)
# plt.plot(yTest)
# plt.title("Actual")
